...
var JotForm = {
    ...
    description: function(input, message) {
        ...
        message = message.replace(/<br>/g, "[br]").replace(/<br\/>/g, "[br]").replace(/<br \/>/g, "[br]").replace(/<p>/g, "[p]").replace(/<\/p>/g, "[/p]").replace(/<b>/g, "[b]").replace(/<\/b>/g, "[/b]").replace(/<strong>/g, "[strong]").replace(/<\/strong>/g, "[/strong]").replace(/<a/g, "[a]").replace(/<\/a>/g, "[/a]").replace(/<img/g, "[img]").replace(/&/g, '&amp;').replace(/</g, '&lt;').stripTags().replace(/\[br\]/g, "<br />").replace(/\[a\]/g, "<a").replace(/\[\/a\]/g, "</a>").replace(/\[p\]/g, "<p>").replace(/\[\/p\]/g, "</p>").replace(/\[b\]/g, "<b>").replace(/\[\/b\]/g, "</b>").replace(/\[strong\]/g, "<strong>").replace(/\[\/strong\]/g, "</strong>").replace(/\[img]/g, "<img")
        ...
        input.validateInput = function(deep) {
            ...
            if (vals.include("required")) {
                if (input.tagName == 'INPUT' && input.readAttribute('type') == "file") {
                    var formInput = input.up('.form-input');
